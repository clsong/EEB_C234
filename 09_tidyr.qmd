...
Hierachical data is extremely common in the real world. For example, when we do the field survey, we have xxxx

It is a very common practice to store hierarchical data in a nested format. For example, the `example_gymnastics_3` dataset contains data on the scores of three countries’ women’s Olympic gymnastic teams in 2012 and 2016. The `scores` column is a list column that contains a tibble for each country.
