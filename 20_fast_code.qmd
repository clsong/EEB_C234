...
library(furrr)
plan(multisession, workers = 4)  # Set up parallel processing with 4 cores
tic()
model_r2 <- flights |> 
  group_by(carrier, origin, year) |> 
  nest() |> 
...
Note: Parallelizing adds some overhead (think of the leader needs time to coordinate the workers). Thus, use it for big datasets or slow tasks—not for quick calculations!
...
model_r2 <- flights |> 
  ...
  \(x) summary(lm(arr_delay ~ dep_delay, data = x))$r.squared),
  .parallel = TRUE) # <2>
toc()
```
