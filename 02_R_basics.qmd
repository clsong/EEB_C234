...
::: {.callout-caution}
## Exercise
1. Create a variable `y` and assign it the value of `5 * 3`.
2. Display the value of `y` on the console.
3. Create a variable `z` and assign it the value of `y / 2`.
:::
...
While loading teh 8 packages is needed and warrants a warning message, it was a poor design principe with the conflicts.

If you do not want to see the messages, you can use the `suppressPackageStartupMessages()` function. For example, you can use the following code to load the `tidyverse` package:
```{r}
