...
While ggplot2 is powerful, it is not a design software. To add extra touch, you can save it as a vector graphic (e.g., `pdf`) and edit it in a design software like Adobe Illustrator or Inkscape. 

However, learning a new software can be time-consuming. Luckily, you get to do many of these tasks in PowerPoint or Keynote. The trick is to save the plot as `svg` format. This is a vector graphic format that can be easily imported into editable format in PowerPoint or Keynote. For example, you can add annotations, arrows, and other design elements to make your plot more informative and appealing. As an example, below adds the artistic plot of penguins to better illustate the two axis:

![Hooray! Our First Publication-Ready Figure](img/final_penguin.png)

Another great side benefit is that that you can esaily do animations for your figures when you present in conferences or group meetings. We will demonstratre in class how to do this.



## Make it interactive (Optional) 
Unfortunbately, acadmeic publishing does not allow interactive plots. However, you can make it interactive for your own use. Below I show you how to make the plot interactive using the `ggiraph` package.
